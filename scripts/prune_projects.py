...
def fix_coords(projects):
    """Repair or remove records that cannot be placed on a map.

    Sharding exposed that 17% of the live file has coordinates outside the
    possible range. Two causes, two different answers:

      SWAPPED   lat/lng transposed (a Russian gas field at lat 98.67, lng 59.04
                is at 59.04, 98.67). Unambiguous when |lat|>90 and |lng|<=90,
                so swap it.
      PROJECTED easting/northing in metres from WFS servers (wfs_es, wfs_be,
                wfs_gb...) - values like 3129980. Converting needs the source
                CRS, which is not in the record, so guessing would scatter
                pins across the wrong continent. These are DROPPED and counted
                per source, so the harvester can be fixed at the point where
                the projection is known.
    """
    kept, swapped, dropped = [], 0, collections.Counter()
    for p in projects:
        ...
        try:
            ...
            dropped[p.get("source", "?")] += 1
            continue
        if abs(la) <= 90 and abs(lo) <= 180:
            kept.append(p)
            continue
        if abs(la) > 90 and abs(la) <= 180 and abs(lo) <= 90:
            p["lat"], p["lng"] = lo, la
            p["coord_fixed"] = "swapped"
            swapped += 1
            kept.append(p)
            continue
        dropped[p.get("source", "?")] += 1
    return kept, swapped, dropped
...
def selftest():
    ...
    rows = [{"lat": 98.6705, "lng": 59.0394, "source": "gem"},      # swapped
            {"lat": 3129980.0, "lng": 17052115.0, "source": "wfs_es"},  # projected
            {"lat": 51.5, "lng": -0.1, "source": "ok"},              # fine
            {"lat": None, "lng": None, "source": "nocoord"}]         # untouched
    kept2, sw, dr = fix_coords(rows)
    eq(len(kept2), 3, "coords/drops-unplaceable-only")
    eq(sw, 1, "coords/swaps-one")
    eq((kept2[0]["lat"], kept2[0]["lng"]), (59.0394, 98.6705), "coords/swap-correct")
    eq(kept2[0]["coord_fixed"], "swapped", "coords/marks-the-fix")
    eq(dr["wfs_es"], 1, "coords/counts-by-source")
    eq(kept2[2]["source"], "nocoord", "coords/keeps-uncoordinated")
...
def main():
    ...
    ap.add_argument("--fix-coords", action="store_true",
                    help="swap transposed lat/lng and drop unplaceable projected coords")
    ...
    if args.fix_coords:
        before = len(projects)
        projects, swapped, dropped_src = fix_coords(projects)
        print(f"coords: swapped {swapped} | dropped {before - len(projects) - swapped} "
              f"unplaceable")
        for src, n in dropped_src.most_common(10):
            print(f"    {n:7d}  {src}")
